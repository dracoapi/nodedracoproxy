...
export default class WebUI {
  ...
  launch() {
    ...
    if (config.active) {
      ...
      app.get('/api/config', <express.RequestHandler>(
        _.bind(this.getConfig, this)
      ));
      app.get('/api/sessions', <express.RequestHandler>(
        _.bind(this.getSessions, this)
      ));
      app.get('/api/session/:session', <express.RequestHandler>(
        _.bind(this.getRequests, this)
      ));
      app.get('/api/request/:session/:request', <express.RequestHandler>(
        _.bind(this.decodeRequest, this)
      ));
      app.get('/api/response/:session/:request', <express.RequestHandler>(
        _.bind(this.decodeResponse, this)
      ));
      ...
      app.listen(config.port, () => {
        logger.info(`UI started, port ${config.port}.`);
      });
    } else {
  ...
  async getSessions(
    ...
    try {
      ...
      const data = await Bluebird.map(folders, async folder => {
        const info = {
          id: folder,
          title: moment(folder, 'YYYYMMDD.HHmmss').format(
            'DD MMM YY - HH:mm:ss'
          )
        };
  ...
  async getRequests(
    ...
    logger.info(`Getting requests for session ${req.params.session}`);
    try {
      ...
      const infos = await Bluebird.map(files, async file => {
        ...
        const data = await this.decoder.decodeRequest(
          req.params.session,
          requestid,
          force
        );
        ...
        if (data.more && data.more.url) {
          if (data.more.url === 'https://us.draconiusgo.com/serviceCall') {
            ...
            data.more.url.startsWith('https://us.draconiusgo.com/AssetBundles')
          ) {
            title = 'asset';
          } else {
  ...
  async decodeRequest(
    ...
    logger.info(
      'Decrypting session %d, request %s',
      req.params.session,
      req.params.request
    );
    try {
  ...
  async decodeResponse(
    ...
    logger.info(
      'Decrypting session %d, response %s',
      req.params.session,
      req.params.request
    );
    try {
      const force = !this.config.ui.cachejson;
      let data = await this.decoder.decodeResponse(
        req.params.session,
        req.params.request,
        force
      );

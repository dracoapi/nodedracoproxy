    ...
    try {
      if (!force && fs.existsSync(`data/${session}/${requestId}.req.json`)) {
        const data = await fs.readFile(
          `data/${session}/${requestId}.req.json`,
          'utf8'
        );
      ...
      const content = await fs.readFile(
        `data/${session}/${requestId}.req.bin`,
        'utf8'
      );
      ...
      if (
        ...
        const request = Wreck.toReadableStream(
  ...
  async decodeResponse(
    ...
    try {
      if (!force && fs.existsSync(`data/${session}/${requestId}.res.json`)) {
        const data = await fs.readFile(
          `data/${session}/${requestId}.res.json`,
          'utf8'
        );
        return JSON.parse(data);
      }
      ...
      let raw = await fs.readFile(
        `data/${session}/${requestId}.res.bin`,
        'utf8'
      );
      if (raw[0] === '{'.charCodeAt(0)) raw = JSON.parse(raw).data;
      const deserializer = new GenericDeserializer(
        version,
      ...
      logger.error(
        `Error decrypting response ${requestId} of session ${session}`
      );
      logger.error(e);
